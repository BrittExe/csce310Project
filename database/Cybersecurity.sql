...
--
-- Host: localhost
-- Generation Time: Nov 26, 2023 at 05:20 PM
...
INSERT INTO `College_Student` (`UIN`, `Gender`, `Hispanic/Latino`, `Race`, `U.S. Citizen`, `First_Generation`, `DoB`, `GPA`, `Major`, `Minor #1`, `Minor #2`, `Expected_Graduation`, `School`, `Classification`, `Phone`, `Student_Type`) VALUES
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `Total_College_Student`  AS SELECT `User`.`UIN` AS `UIN`, `User`.`First_Name` AS `First_Name`, `User`.`M_Initial` AS `M_Initial`, `User`.`Last_Name` AS `Last_Name`, `User`.`Username` AS `Username`, `User`.`Passwords` AS `Passwords`, `User`.`User_Type` AS `User_Type`, `User`.`Email` AS `Email`, `User`.`Discord_Name` AS `Discord_Name`, `User`.`Is_Deleted` AS `Is_Deleted`, `College_Student`.`Gender` AS `Gender`, `College_Student`.`Hispanic/Latino` AS `Hispanic/Latino`, `College_Student`.`Race` AS `Race`, `College_Student`.`U.S. Citizen` AS `U.S. Citizen`, `College_Student`.`First_Generation` AS `First_Generation`, `College_Student`.`DoB` AS `DoB`, `College_Student`.`GPA` AS `GPA`, `College_Student`.`Major` AS `Major`, `College_Student`.`Minor #1` AS `Minor #1`, `College_Student`.`Minor #2` AS `Minor #2`, `College_Student`.`Expected_Graduation` AS `Expected_Graduation`, `College_Student`.`School` AS `School`, `College_Student`.`Classification` AS `Classification`, `College_Student`.`Phone` AS `Phone`, `College_Student`.`Student_Type` AS `Student_Type` FROM (`User` join `College_Student` on(`User`.`UIN` = `College_Student`.`UIN`)) ;
